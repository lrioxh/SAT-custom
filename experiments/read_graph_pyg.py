...
def read_csv_heterograph_raw(raw_dir, add_inverse_edge = False, additional_node_files = [], additional_edge_files = []):
    ...
    * edge_feat_dict and node_feat_dict are optional: if a graph does not contain it, we will simply have None.
    ...
    # loading necessary files
    try:
        num_node_df = pd.read_csv(osp.join(raw_dir, 'num-node-dict.csv'))
        ...
        triplet_df = pd.read_csv(osp.join(raw_dir, 'triplet-type-list.csv'), header = None)
        ...
        for triplet in triplet_list:
            ...
            edge_dict[triplet] = pd.read_csv(osp.join(subdir, 'edge.csv'), header = None).values.T.astype(np.int64)
            num_edge_dict[triplet] = pd.read_csv(osp.join(subdir, 'num-edge-list.csv'), header = None).astype(np.int64)[0].tolist()
    ...
    for nodetype in nodetype_list:
        ...
        try:
            node_feat = pd.read_csv(osp.join(subdir, 'node-feat.csv'), header = None).values
            if 'int' in str(node_feat.dtype):
                node_feat = node_feat.astype(np.int64)
            else:
                # float
    ...
    for triplet in triplet_list:
        ...
        try:
            edge_feat = pd.read_csv(osp.join(subdir, 'edge-feat.csv'), header = None).values
            if 'int' in str(edge_feat.dtype):
                edge_feat = edge_feat.astype(np.int64)
            else:
                #float
    ...
    for additional_file in additional_node_files:
        ...
        for nodetype in nodetype_list:
            ...
            try:
                node_feat = pd.read_csv(osp.join(subdir, additional_file + '.csv'), header = None).values
                if 'int' in str(node_feat.dtype):
                    node_feat = node_feat.astype(np.int64)
                else:
                    # float
    ...
    for additional_file in additional_edge_files:
        ...
        for triplet in triplet_list:
            ...
            try:
                edge_feat = pd.read_csv(osp.join(subdir, additional_file + '.csv'), header = None).values
                if 'int' in str(edge_feat.dtype):
                    edge_feat = edge_feat.astype(np.int64)
                else:
                    # float
...
def read_node_label_hetero(raw_dir):
    df = pd.read_csv(osp.join(raw_dir, 'nodetype-has-label.csv'))
    label_dict = {}
    for nodetype in df.keys():
        has_label = df[nodetype].values[0]
        if has_label:
            label_dict[nodetype] = pd.read_csv(osp.join(raw_dir, 'node-label', nodetype, 'node-label.csv'), header = None).values
...
def read_nodesplitidx_split_hetero(split_dir):
    df = pd.read_csv(osp.join(split_dir, 'nodetype-has-split.csv'))
    train_dict = {}
    valid_dict = {}
    test_dict = {}
    for nodetype in df.keys():
        has_label = df[nodetype].values[0]
        if has_label:
            train_dict[nodetype] = pd.read_csv(osp.join(split_dir, nodetype, 'train.csv'), header = None).values.T[0]
            valid_dict[nodetype] = pd.read_csv(osp.join(split_dir, nodetype, 'valid.csv'), header = None).values.T[0]
            test_dict[nodetype] = pd.read_csv(osp.join(split_dir, nodetype, 'test.csv'), header = None).values.T[0]
