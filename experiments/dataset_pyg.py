...
class PygGraphPropPredDataset(InMemoryDataset):
    def __init__(self, name, root = 'dataset', transform=None, pre_transform = None, meta_dict = None):
        ...
        if meta_dict is None:
            ...
            master = pd.read_csv(os.path.join(root, 'datasets.csv'), index_col=0, keep_default_na=False)
    ...
    def process(self):
        ...
        if self.task_type == 'subtoken prediction':
            graph_label_notparsed = pd.read_csv(osp.join(self.raw_dir, 'graph-label.csv'), header = None).values
